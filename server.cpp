...
int main(int argc, char* argv[]) {
  ...
  /*  vector<string> parameters = lexer(argv[1]);
    ...
    try {
        ...
        fstream text;
        text.open("/home/fanny/Documents/Advance conding/FlightSimulator-master26.12.19/FlightSimulator-master/Part 1/fly.txt", ios::in | ios::binary);
        if (!text) {
    ...
    /*  vector<string> parameters = lexer(text);
      return 0;*/
    openDataServer();
    thread thread_server(openServer, 5400);
    thread thread_client(connectClient, 5402);
    thread_client.join();
    thread_server.join();
...
bool xmlParser() {
    ...
    unordered_map<string, typename list<Variable>::iterator> *vars_map = new unordered_map<string,
